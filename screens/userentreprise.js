...
const Userentreprise = ({ route }) => {
  ...
  const { id: Idpropre } = state;
    ...
    const suprimerentreprise = () => {


        axios
            .delete(`${API_URL}supprimer-entreprise/${Idpropre}/${userId}`)
            .then((res) => {
    ...
    const ajouterentreprise = (Id) => {
        ...
        axios
            .post(`${API_URL}ajouter-entreprise/${Idpropre}/${userId}`)
            .then((res) => {
                console.log(res.data);
                if (res.data.status == "Ok") {
                    Toast.show({
    ...
    useEffect(() => {
        ...
        const fetchUser2 = async () => {
            try {
                const response = await fetch(`${API_URL}user/${Idpropre}`);
                console.log(response)
                ...
                console.log('data.user2',data.user)
            } catch (error) {
                console.error('Error fetching profile:', error);
            }
        };
        ...
        <SafeAreaView
            ...
            {showOptions && (
              <View transparent style={styles.optionsContainer}>
                ...
                <TouchableOpacity onPress={()=>{  navigation.goBack()}} style={{ marginVertical: 0 }}>
                ...
                <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'left' }}>
                    ...
                    <Pressable
                        ...
                        onPress={() => {
                            if (profile2 && profile2.entreprises) {
                                if (profile2.entreprises.includes(userId)) {
                                    suprimerentreprise();
                                } else {
                                    ajouterentreprise();
                                }
                       ...
                       >
                        ...
                        <Text style={{ textAlign: "center", fontWeight: "bold", fontSize: 16, color: tem.text }}>
                        {profile2 && profile2.entreprises && profile2.entreprises.includes(userId) ? "Suivi" : "Suivre"}
